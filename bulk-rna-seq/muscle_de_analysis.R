...
setwd("C:/Users/dmich/Dropbox (Personal)/CBDM Lab/Data/MEC_RNA-seq/2021-08-19_spib_ckm_rnaseq/Batch116_Dan_Mouse")

date = "2021-10-15"

norm_table <- read.delim( "Batch116_Mouse_Dan_1_adjust.gct", sep="\t", header=T, skip=2, row.names=1 )
...
pdf( paste0("figures/pairs_ckmthy_",date,".pdf"), height=7, width=7 )
pairs(log2(norm_table[,1:4]))
dev.off()

pdf( paste0("figures/pairs_muscle_",date,".pdf"), height=5, width=5 )
pairs(log2(norm_table[,5:6]))
dev.off()

pdf( paste0("figures/pairs_ckmthy_muscle_",date,".pdf"), height=10, width=10 )
pairs(log2(norm_table[,1:6]))
dev.off()
...
#examine PC scores
pdf( paste0("figures/PCA_plot_cleaned_", date, ".pdf" ), height=2.75, width=4 )
for (i in 1:4) {
...
muscle_sig <- read.delim("C:/Users/dmich/Dropbox (MIT)/CBDM Lab MIT/Data/MEC_scRNA/DAM_082620/text_outputs/2021-09-16_sig_de/signatures/20210924_Muscle_sig_logfc1_pct10_qval0.01.txt", header=F, sep="\t")
muscle_sig <- muscle_sig$V1

signature <- as.vector(muscle_sig)
...
pdf(paste0("figures/",i,"_vs_", j,"_volcano_muscle_bespoke_sig_new_lab_", date ,".pdf"), height=4, width=6)
ggplot( qlf$table, aes( x=-qlf$table$logFC, y=-log10(qlf$table$PValue) ) ) +
  geom_point( aes(color=color_idx) ) +
  geom_point( data=qlf$table[signature,], aes( x=-qlf$table[signature,"logFC"], 
                                               y=-log10(qlf$table[signature,"PValue"]) ),
              color="purple" ) +
  geom_text_repel( label=label_idx, color="black", segment.alpha=0.75, segment.color="gray", segment.size=0.25, max.overlaps=100 ) +
  xlab(paste0("log2 fold change, ", i, " vs ", j)) +
  ylab("-log10 P-value") +
  ...
  annotate( geom="text", label=sum(color_idx=="signature+"&qlf$table$logFC<0), x=4, y=0.5, color="purple" ) +
  annotate( geom="text", label=sum(color_idx=="signature+"&qlf$table$logFC>0), x=-4, y=0.5, color="purple" )
dev.off()
